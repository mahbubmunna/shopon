...
class MessageProvider extends ChangeNotifier {
  ...
  Future<Conversations> allConversations() async {
    ...
    await http
        .get("${api_base_url}conversations", headers: header)
        .then((value) {
  ...
  Future<Message> startConversetion(product_id, title, messages) async {
    Message message;
    ...
    await http
        .post("${api_base_url}start-conversation", body: body, headers: header)
        .then((value) {
      print("The Value Iss ${value.body}");
      message = Message.fromJson(json.decode(value.body));
    });
